...
REPOSITORY ?= salmagundi/keuzewijzeraardgasvrij-frontend
VERSION ?= latest
...
manifests:
	@helm template keuzewijzeraardgasvrij-frontend $(HELM_ARGS) $(ARGS)

deploy: manifests
	helm upgrade --install keuzewijzeraardgasvrij-frontend $(HELM_ARGS) $(ARGS)
...
reset:
	helm uninstall keuzewijzeraardgasvrij-frontend
